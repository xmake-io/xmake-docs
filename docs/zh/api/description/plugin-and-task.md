# 插件任务 {#plugin-and-task}

Xmake 可以实现自定义任务或者插件，其两者的核心就是`task`任务，其两者实际上是一样的，xmake的插件都是用`task`实现的。

本质上都是任务，只是[set_category](#set_category)分类不同而已。

如需扩展构建流程而非定义独立任务，请参阅[自定义规则](/zh/api/description/custom-rule)。插件开发的入门教程请参阅[插件开发指南](/zh/guide/extensions/plugin-development)。
