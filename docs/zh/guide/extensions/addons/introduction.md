...
**载荷是带命名空间的。** 规则、工具链、includes 和模块统一通过 `@addon/<name>/...` 引用，所以两个 addon 永远不会撞车。插件和模板是例外 —— 命令名是全局的，因此 xmake 会拒绝那些会覆盖别的 addon 命令的安装。
...
| Addon | `xmake addon --install` / `add_addons` | `~/.xmake/addons` | xmake，所有载荷类型 |
...
- [编写 Addon](/zh/guide/extensions/addons/development) —— manifest、载荷和提交到 xmake-repo
